...
function AthenaOIDCFields(props: { isEditMode: boolean }) {
  ...
  return (
    <>
      ...
      >
        <Input.Password
          placeholder="OAuth 2.0 访问令牌或 OpenID Connect ID 令牌"
          autoComplete="off"
        />
      </Form.Item>
      ...
      <Form.Item
        ...
        extra="可选的会话名称，用于 AWS STS AssumeRole 操作。"
      >
  ...
  return (
    <>
      ...
      <Form.Item
        label="数据库（Schema）"
        name="schema"
        extra="包含目标数据表的 Athena 数据库（Schema）。"
        required
        rules={[
          {
            required: true,
      ...
      <Form.Item
        ...
        extra={
          <>
            Athena 用于存储查询结果和元数据的 S3 路径。
            <br />
            你可以在 Athena 控制台的 <b>设置 → 查询结果位置</b> 中查看。
          </>
        }
        rules={[
          {
