...
- Denodo rate prompt guidance: prompt rules, default metric formulas, runtime metric formulas, and the private full-lead conversion/refund-comparison hidden exemplar should preserve the formula's expression shape and should not add metric-calculation casts unless the formula itself explicitly contains them. Current customer-calibrated formulas prefer direct expressions such as `COUNT(...) * 100.0 / NULLIF(...)`.
