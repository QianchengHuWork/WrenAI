...
- Current Denodo rounding fix from this chat: the hidden full-lead conversion exemplar now uses `DECIMAL(18, 6)` / `CAST(100 AS DECIMAL(18, 6))`, and the UI Denodo native SQL / direct DIALECT sanitizer no longer converts count-based conversion-rate casts back to `FLOAT`. Final SQL sent to Denodo MCP should keep rate/percentage `ROUND(expr, 2)` math as DECIMAL to avoid `round(double precision, integer)`.
...
| Current chat - Denodo hidden exemplar rounding fix | 2026-05-22 16:48 CST | Done | Denodo prompt exemplar / rounding precision hardening. | Fixed the hidden full-lead conversion exemplar in `wren-ai-service/src/pipelines/generation/denodo_prompt_context.py` to use `DECIMAL(18, 6)` and `CAST(100 AS DECIMAL(18, 6))` instead of `100.0`, so the exemplar no longer nudges the model toward `ROUND(... * FLOAT, 2)` / `round(double precision, integer)`. Added regression coverage in `tests/pytest/pipelines/generation/test_denodo_prompt_context.py` and verified with `poetry run pytest tests/pytest/pipelines/generation/test_denodo_prompt_context.py -q` (19 passed). |
...
- Denodo rounded rate/percentage expressions now require `DECIMAL(18, 6)` casts rather than `FLOAT`; this avoids PostgreSQL/JDBC pushdown failures like `round(double precision, integer)` when using `ROUND(expr, 2)`.
...
- Re-run the user-edited smart-assignment conversion question and confirm the reasoning/output uses `is_conver_order = 1` plus `CAST("actual_price" AS DECIMAL(18, 2))`, not the built-in `is_ord_fdpay` fallback.
