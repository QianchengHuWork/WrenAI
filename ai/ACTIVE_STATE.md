...
Last updated: 2026-05-27 15:47 CST
...
- Latest scope correction: business confirmed `线索等级 -> 智能分配 -> dv_assign_total_conversion_core`. Scope prompt and metric formulas now treat lead-level/different-level conversion or output amount questions as smart-assignment domain, and no longer include lead level in the line-clue overview formula scope.
...
| Current chat - Lead-level scope correction | 2026-05-26 13:51 CST | Done | Denodo scope prompt and metric formula hints. | Business chain confirmed as `线索等级 -> 智能分配 -> dv_assign_total_conversion_core`. Updated scope-resolution prompt, default metric formulas, and runtime formula file so different-lead-level conversion/output amount questions prefer `dv_assign_total_conversion_core` instead of `dv_clew_core` + `dv_ord_core`. Verified focused AI pytest, UI typecheck, JSON parse, and `git diff --check`. |
...
- The default metric-formula layer now treats assigned-store cross-region purchase questions as smart-assignment conversion questions: use `dv_assign_total_conversion_core`, group by `fac_name`, judge cross-region purchase with `is_cross_order = 1`, and avoid `dv_clew_assign_core`.
...
- Prompt-injection inventory moved to `docs/customer-facing/denodo-prompts/PROMPT_INJECTION_MAP.md`; verified by static source inspection only, no service behavior tests run.
- Chinese Denodo prompt-layer inventory moved to `docs/customer-facing/denodo-prompts/DENODO_PROMPT_LAYERS.zh-CN.md`; it is customer-facing and redacts internal product/system naming, code source locations, SQL-pair terminology/content, and hidden SQL exemplar SQL. It now also explains metric calculation platform rule injection, including view-selection influence, SQL-generation optimization, and injection timing. Documentation-only; no service behavior tests run.
- Independent customer-facing Denodo prompt reference moved to `docs/customer-facing/denodo-prompts/DENODO_PROMPT_REFERENCE.zh-CN.md`; it includes implementation-source references and a complete complex-planning CTE structure while keeping historical reference examples as placeholders and avoiding private mechanism terminology. Documentation-only; no service behavior tests run.
- The Denodo prompt reference now includes all current enabled metric calculation prompt rules: smart-assignment conversion, lead-overview conversion, smart-assignment test-drive conversion, and package-order metrics, including scope, triggers, expressions, forbidden patterns, and extra instructions. Documentation-only; no service behavior tests run.
...
- Re-run the assigned-store cross-region purchase question with anchored date `2026-03-18` and confirm selected model is `dv_assign_total_conversion_core`, time filters map previous month to `202602`, and SQL uses `fac_name`, `is_cross_order = 1`, `is_conver_order = 1`, and `actual_price`.
