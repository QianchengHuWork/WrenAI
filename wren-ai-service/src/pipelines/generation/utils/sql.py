...
- For Denodo rate, ratio, percentage, conversion rate, success rate, coverage rate, refund rate, share, or any metric calculated as numerator / denominator from counts or summed counts, do not use CAST(... AS FLOAT) or DOUBLE inside ROUND(expr, scale). Denodo/JDBC pushdown can reject `round(double precision, integer)`. For rounded rates or percentages, cast numerator and denominator to DECIMAL(18, 6), use NULLIF on the denominator, and multiply by CAST(100 AS DECIMAL(18, 6)) when the metric is a percentage. Keep DECIMAL for monetary amount calculations and numeric text conversions.
...
7. MANDATORY PARTITION FILTERING: If any of the queried tables contain columns named `ptstart` and `ptend`, you MUST explicitly plan to include them in the `WHERE` clause to restrict the data scan. Plan to set `ptstart` and `ptend` to the start and end dates of the query range in `YYYYMMDD` format with equality predicates only, for example `ptstart = '20240501' AND ptend = '20240531'`. Do NOT plan <, <=, >, >=, or BETWEEN for `ptstart` or `ptend`. If you use `ptstart` and `ptend`, you MUST NOT plan to use or filter by the `pt` column.
